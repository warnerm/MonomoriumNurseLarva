...
#Constraint
f.est <- read.csv("~/GitHub/popgenAM/data/MKtestConstraintOneAlpha.csv")
...
df <- read.table("~/GitHub/MonomoriumNurseLarva/data/phylostratigraphy_codes.txt",sep="\t")
names(df) = c("Species","code","NCBI Taxonomy ID")
df = df[,-c(2)]
write.table(df,"~/GitHub/MonomoriumNurseLarva/data/phylostratigraphy_species.txt",row.names = FALSE)
